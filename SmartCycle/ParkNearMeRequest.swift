...
class ParkNearMeRequest  {
    typealias callbackBlock = ((_ results: [ParkItem]?, _ error: Error?) -> Void)

    fileprivate var callback: callbackBlock?
    let reachability = Reachability()!

    func execute(location: CLLocation, radius: Float, userId: String, _ callback: @escaping ([ParkItem]?, Error?) -> Void) {
        ...
        let serviceUrl = String(format:"%@parking/listNearMe", Constants.Urls.SmartcycleAPIUrl)
...
//            var backToString = String(data: data!, encoding: String.Encoding.utf8) as String!
            ...
            do {
                ...
                var results: [ParkItem] = []

                for subJson in jsonResponse.array! {
                    let item = ParkItem(json: subJson)
                    results.append(item)
                }
